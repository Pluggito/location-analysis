"use client"
import { MapContainer, TileLayer, Popup, useMapEvents, Circle, Marker, Polyline } from "react-leaflet"
...
export default function ZoningPPage() {
  ...
  const ports = [
    ...
    { name: "GCT New York Terminal", coords: [ 40.6375182,-74.3377233], type: "Port" },
  ...
  const fetchTravelTime = async (originCoords: L.LatLngTuple, destination: [number, number]): Promise<RouteData> => {
    try {
  ...
  const handleDirectionsClick = (type: "port" | "airport" | "highway", index: number) => {
                ...
                <Marker key={`port-${idx}`} position={coords as L.LatLngExpression} icon={portIcon}>
                  <Popup>
                    ...
                    <span>
                      ...
                      {isCalculating
                        ? "Calculating..."
                        : portTravelData[idx]?.duration && portTravelData[idx].duration !== -1
                          ? portTravelData[idx].duration > 60
                            ? `${Math.round(portTravelData[idx].duration / 60)} mins`
                    ...
                    <span>
                      ...
                      {isCalculating
                        ? ""
                        : portTravelData[idx]?.distance && portTravelData[idx].distance > 0
                          ? `${(portTravelData[idx].distance / 1000).toFixed(2)} km`
                          : "Not available"}
                    </span>
                    <br />
                    ...
                    <button
                      ...
                      className={`mt-2 px-2 py-1 text-white text-xs rounded hover:bg-green-700 ${
                        selectedRoute?.type === "port" && selectedRoute?.index === idx ? "bg-green-700" : "bg-green-600"
                      }`}
                    >
                ...
                <Marker key={`airport-${idx}`} position={coords as L.LatLngExpression} icon={airportIcon}>
                  <Popup>
                    ...
                    <span>
                      ...
                      {isCalculating
                        ? "Calculating..."
                        : airportTravelData[idx]?.duration && airportTravelData[idx].duration !== -1
                          ? airportTravelData[idx].duration > 60
                            ? `${Math.round(airportTravelData[idx].duration / 60)} mins`
                    ...
                    <span>
                      ...
                      {isCalculating
                        ? ""
                        : airportTravelData[idx]?.distance && airportTravelData[idx].distance > 0
                          ? `${(airportTravelData[idx].distance / 1000).toFixed(2)} km`
                          : "Not available"}
                    </span>
                    <br />
                    <button
                    ...
                    >
                      {selectedRoute?.type === "airport" && selectedRoute?.index === idx ? "Hide Route" : "Directions"}
                    </button>
                  </Popup>
                </Marker>
              ))}
                  ...
                  <Popup>
                    <div className="text-center ">
                      ...
                      <span>
                        ...
                        {isCalculating
                          ? "Calculating..."
                          : highwayTravelData[idx]?.duration && highwayTravelData[idx].duration !== -1
                            ? highwayTravelData[idx].duration > 60
                              ? `${Math.round(highwayTravelData[idx].duration / 60)} mins`
                      ...
                      <span>
                        ...
                        {isCalculating
                          ? ""
                          : highwayTravelData[idx]?.distance && highwayTravelData[idx].distance > 0
                            ? `${(highwayTravelData[idx].distance / 1000).toFixed(2)} km`
                            : "Not available"}
                      </span>
                      <br />
                      <button
